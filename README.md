...
| `serve.py` | кросс-платформенный запуск веб-GUI на локальном сервере (Windows/Unix) |

## Веб-GUI (`ui.html`)
Самый надёжный способ — локальный сервер (все библиотеки отдаются рядом с `ui.html`):
```bash
python serve.py            # открывает http://127.0.0.1:8123/ui.html
python serve.py 8080       # другой порт
python serve.py --stop     # остановить сервер
```
Работает на Windows, Linux, macOS (только Python 3). GUI — компактный конвертер
`.zip → подписанный .sfs` + панель ключей и проверки. Можно и просто открыть
`ui.html` файлом (`ed25519.js`, `parser.js`, `vendor/`, `zip-dec.js` должны лежать
рядом), но при `file://` некоторые браузеры ограничивают загрузку локальных
ресурсов — тогда распаковка ZIP вернёт ошибку вида «не загружены распаковщики
(zip-dec.js / vendor/)».

Возможности:
- **конвертация `.zip` → подписанный `.sfs`** (главный экран): перетащить архив,
  одна кнопка — результат `converted.sfs` с подписью Ed25519;
- упаковка/проверка `.sfs`, экспорт/импорт ключей издателя (.json, .pub),
  allowlist доверенных издателей.
